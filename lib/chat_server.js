...
function handleNameChangeAttempts(socket,nickNames,namesUsed) {
  ...
  // Don't allow nicknames to being with Guest
    if (name.indexOf('Guest') === 0 || name.indexOf('You') === 0 || name.indexOf('you') === 0 ) {
      socket.emit('nameResult',{
        success: false,
        message: "Names cannot start with 'Guest' or 'You'"
      });
    } else {
...
function handleMessageBroadcasting(socket) {
  ...
  // Send message to person typing to prefix message with you
    socket.emit('message',{
