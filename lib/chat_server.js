...
// The application then commuincate related details to the user and other users in the same romo
...
function handleNameChangeAttepts(socket,nickNames,namesUsed) {
  ...
  // Get current users previous name and index if it has one
        var previousName = nickNames[socket.id];
        var previousNameIndex = namesUsed.indexOf(previousName);
  // Add name to namesused
        namesUsed.push(name);
        nickNames[socket.id] = name;
  // Remove previous name to make room available to other clients
        delete namesUsed[previousNameIndex];
  // Emit name change
        socket.emit('namedResult',{
          success: true,
          name: name
        });
  // Broadcast to everyone in room of name change
        socket.broadcst.to(currentRoom[socket.id]).emit('message',{
